...
class DataValidator {
    static validateRecord(record, config) {
        ...
        const requiredValidation = validationUtils_1.ValidationUtils.validateRequiredFields(record, config.requiredFields, config.allowEmptyFields);
        ...
        for (const field of emailFields) {
            if (record[field] && !validationUtils_1.ValidationUtils.validateEmail(record[field])) {
        ...
        for (const field of dateFields) {
            if (record[field] && !validationUtils_1.ValidationUtils.validateDate(record[field])) {
        ...
        for (const field of numericFields) {
            if (record[field] !== undefined && record[field] !== '' && !validationUtils_1.ValidationUtils.validateNumber(record[field])) {
